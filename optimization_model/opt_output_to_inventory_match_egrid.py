...
gen = feather.read_dataframe('/home/gchossie/epa_ace/optimization_model/outputs/gen_tst_jan_29.feather')
carac = pd.read_csv('/home/gchossie/epa_ace/optimization_model/good_model_inputs/inputs_gen_tst_jan_29.csv')
...
# Feb 14: Match eGRID
no = pd.DataFrame(data=opt_out['PLNOXRTA'].values.reshape(len(opt_out.index),1) * opt_out['NOX corr'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
...
no2 = pd.DataFrame(data=opt_out['PLNOXRTA'].values.reshape(len(opt_out.index),1) * opt_out['NOX corr'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
# no is now in lb/s
...
no2[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] *= mult
# no2 is now in kg/s

so2 = pd.DataFrame(data=opt_out['PLSO2RTA'].values.reshape(len(opt_out.index),1) * opt_out['SO2 corr'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
# so2 is now in lb/s
...
f_out = f'/net/fs03/d1/gchossie/us_power_grid/annual_emissions/inventory_power_plants_match_egrid.nc'
...
for var in var_list:
    ...
    nc_var.long_name = f'US {var} power plant emissions from my optimization mode'
    src_data = trad[var]
