...
carac = pd.read_csv(f'./good_model_inputs/inputs_gen_no-nuclear_all-generators_20k.csv')
...
# Feb 14: Match eGRID
no = pd.DataFrame(data=opt_out['PLNOXRTA'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
...
no2 = pd.DataFrame(data=opt_out['PLNOXRTA'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
# no is now in lb/s
...
so2 = pd.DataFrame(data=opt_out['PLSO2RTA'].values.reshape(len(opt_out.index),1) * opt_out[[f'2017_{day}_{hour}' for day in range(1,366) for hour in range(24)]] / 3600)
# so2 is now in lb/s
