...
run_name = "base"
# Load model inputs
gen = CSV.read(folder * "inputs_gen_$(run_name).csv") #change
load = CSV.read(folder * "inputs_load_$(run_name).csv")
renCF = CSV.read(folder * "inputs_renewableCF.csv")
trans = CSV.read(folder * "inputs_trans_$(run_name).csv")#change
...
if termination_status(m) == MOI.OPTIMAL
    ...
    CSV.write("./outputs/gen_$(run_name)_$(ARGS[1])_$(ARGS[2]).csv", genOut)
    CSV.write("./outputs/trans_$(run_name)_$(ARGS[1])_$(ARGS[2]).csv", transOut)
elseif termination_status(m) == MOI.TIME_LIMIT && has_values(m)
    ...
    CSV.write("./outputs/subopt_gen_$(run_name)_$(ARGS[1])_$(ARGS[2]).csv", suboptimal_genOut)
    CSV.write("./outputs/subopt_trans_$(run_name)_$(ARGS[1])_$(ARGS[2]).csv", suboptimal_transOut)
else
