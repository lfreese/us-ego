...
proper_model_names_dict = {'renew_nonuc_NA':'No Nuclear + \n Renewables', 'nonuc_NA':'No Nuclear','normal_NA': 'Base','egrid_NA':'eGRID','epa_NA':'NEI 2016','nei_NA':'NEI 2011', 'nonuc_coal_NA':'No Nuclear + \n No Coal', 'EPA':'AQS','IMPROVE':'IMPROVE'}
...
def concentration_plot_seasonal_dif_models(ds_seasonal_pm, ds_o3_winter_only, polls, mod_base, mod_deltas, rows, 
                       columns, figsize, levels,
                     cmap,
                       lat_lon, extension = 'both'):
    '''plotting annual mean for pollutants, except ozone which is summertime mean'''
    fig, axes =  plt.subplots(rows, columns, figsize=figsize,subplot_kw={'projection':ccrs.LambertConformal()})
    for idx_poll, poll in enumerate(polls):
        ...
        #make the plot 
            if poll == 'O3':
                q = (ds_o3_winter_only.sel( model_name = mod)-ds_o3_winter_only.sel(model_name = mod_base)).mean(dim = 'time').plot(ax=ax, #set the axis
                           levels = np.squeeze(levels), #set the levels for our colorbars
                           extend=extension,#extend the colorbar in both directions
                           transform=ccrs.PlateCarree(), #fit data into map
                            cmap=cmap, add_colorbar = False)  #choose color for our colorbar
                ...
                q = (ds_seasonal_pm[poll].sel( model_name = mod)-ds_seasonal_pm[poll].sel(model_name = mod_base)).mean(dim = 'season').plot(ax=ax, #set the axis
                                           levels = np.squeeze(levels), #set the levels for our colorbars
                                           extend=extension,#extend the colorbar in both directions
                                           transform=ccrs.PlateCarree(), #fit data into map
                                            cmap=cmap, add_colorbar = False)  #choose color for our colorbar
...
def plant_region_plot(ds, xvariable, yvariable1, egrid, egrid_yvariable, figsize, nocoal,  noxlim = True, normal = True, nonuc = True, renew = True):
    ...
    if noxlim == True:
        plt.bar(ds.sel(model_name = 'nox_lim_model')[xvariable], ds.sel(model_name = 'nox_lim_model')[yvariable1], color = nox_lim_color, width = width, align="edge", label = 'NOx Emissions Limited Model')
    if nocoal == True:
        plt.bar(ds.sel(model_name = 'nonuc_nocoal_model')[xvariable], ds.sel(model_name = 'nonuc_nocoal_model')[yvariable1], color = nocoal_color, width = width, align="center", label = 'No Nuclear + No Coal Model')
    if nonuc == True:
        plt.bar(ds.sel(model_name = 'nonuc_model')[xvariable], ds.sel(model_name = 'nonuc_model')[yvariable1], color = nonuc_color, width = width, align="edge", label = 'No Nuclear')
    if egrid == True:
        plt.bar(ds.sel(model_name = 'normal_model')[xvariable], ds.sel(model_name = 'normal_model')[egrid_yvariable], color = egrid_color, width = -width, align="edge", label = 'Egrid')
    if normal == True:
        plt.bar(ds.sel(model_name = 'normal_model')[xvariable], ds.sel(model_name = 'normal_model')[yvariable1], color = normal_color, width = width, align="center", label = 'Base Model')
    if renew == True:
        plt.bar(ds.sel(model_name = 'nonuc_renewables_model')[xvariable], ds.sel(model_name = 'nonuc_renewables_model')[yvariable1], color = renew_color, width = width, align="edge", label = 'No Nuclear + Renewables Model')

    plt.xticks(rotation = 45)
    ax.legend();
    
def fossil_fuel_plot(ds, sci_names, xvariable, pollutants, figsize, nonuc_color, normal_color,egrid_color, nocoal, egrid, normal = True, renew = True):
    ...
    for ax, pollutant in zip(axes.flatten(), pollutants):
        ...
        if nocoal == True:
            ax.bar(ds.sel(model_name = 'nonuc_nocoal_model').sel(fueltype = ['Coal', 'NaturalGas'])[xvariable], ds.sel(model_name = 'nonuc_nocoal_model').sel(fueltype = ['Coal', 'NaturalGas'])[f'model_annual_{pollutant}_conc']/1000, color = nocoal_color, width = 1+ width, align="center", label = 'No Nuclear + No Coal Model')
        if normal == True:
        ...
        if renew == True:
            ax.bar(ds.sel(model_name = 'nonuc_renewables_model').sel(fueltype = ['Coal', 'NaturalGas'])[xvariable], ds.sel(model_name = 'nonuc_renewables_model').sel(fueltype = ['Coal', 'NaturalGas'])[f'model_annual_{pollutant}_conc']/1000, color = renew_color, width = 3+ width, align="edge", label = 'No Nuclear + Renewables')
